...
function App() {
  ...
  const fetchDailyData = async (lat, lon) => {
    try {
      let uri = `https://api.openweathermap.org/data/3.5/onecall?lat=${lat}&lon=${lon}&exclude=hourly,minutely&units=metric&appid=${process.env.REACT_APP_API_KEY}`;
      await axios.get(uri).then((res) => {
        // console.log(res.data);
